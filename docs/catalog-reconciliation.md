...
## Open items — need a decision, not a code change

1. **`CSCC04` and `CSCC05` duration: source document is internally inconsistent.** In all three region files, the Distribution/Transmission matrix sheet lists both courses as **5 days**, but the pricing sheet in the *same file* prices them at **4 days** (`CSCC04`: SAR 2,000 = 4 × 500; `CSCC05`: SAR 1,800 = 4 × 450) — the actual charged price only makes arithmetic sense at 4 days. The seed currently uses 4 days, matching the pricing sheet. Recommend keeping this unless told the matrix's "5" is the correct figure and the pricing sheet itself needs correcting (which would also change the SAR amounts).
2. **North region has no source document at all.** Only Central/East/South-West files exist. `REGIONAL_DAY_RATES.North` is currently a placeholder defaulted to the Central rate (500 SAR/day). Needs either a real North rate from the client or an explicit sign-off that the placeholder stands until one exists.
3. **19 of 43 courses have no explicit price row in any region's pricing sheet** (all `CTCT*` technical certification tests, plus `CSCC17/24/27/28/29`). Their pricing is extrapolated using the same `duration_days × regional_day_rate` formula confirmed exact for the other 22 — a reasonable assumption given it holds without exception elsewhere, but not directly source-verified for these 19. Flagging for awareness, not blocking.
