...
export default async function CatalogPage({
  ...
  const courses = authorize("manage_catalog", context) ? await listCourses() : [];
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-muted-foreground">
                ...
                <th className="p-3 text-start font-medium">{t("tableDuration")}</th>
              ...
              {courses.length === 0 ? (
                ...
                courses.map((course) => (
                  <tr key={course.id} className="border-b border-border last:border-0">
                    ...
                    <td className="p-3">{locale === "ar" ? course.titleAr : course.titleEn}</td>
                    <td className="p-3">{course.durationHours}</td>
                    <td className="p-3">{course.active ? t("statusActive") : t("statusInactive")}</td>
                    <td className="p-3">
                      <Link href={`/superadmin/catalog/${course.id}`} className="text-primary hover:underline">
                        {t("view")}
                      </Link>
                    </td>
                  </tr>
